...
function fetchAirQualityData(city) {
    const url = `https://api.waqi.info/feed/${city}/?token=${WAQI_API_KEY}`;
    return fetch(url)
        .then(response => response.json())
...
function openPopup(data) {
    ...
    document.getElementById('water-quality').textContent = data.waterQualityScore !== null ? `${data.waterQualityScore}/100` : 'N/A';
